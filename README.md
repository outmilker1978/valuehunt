# ValueHunt — Персональный ассистент поиска работы

[![Python](https://img.shields.io/badge/python-3.12-blue)](https://python.org)
[![FastAPI](https://img.shields.io/badge/FastAPI-0.104+-green)](https://fastapi.tiangolo.com)
[![License](https://img.shields.io/badge/license-MIT-lightgrey)](LICENSE)
[![Release](https://img.shields.io/badge/release-v1.14.0-orange)](https://github.com/outmilker1978/ValueHunt/releases)

**ValueHunt** — локальный open-source инструмент, который автоматизирует поиск работы:
собирает вакансии с HeadHunter, оценивает по твоей личной матрице ценностей и помогает управлять воронкой откликов.

## Быстрый старт

1. [Скачай последний релиз](https://github.com/outmilker1978/ValueHunt/releases) (`ValueHunt.zip`)
2. Распакуй в любую папку
3. Запусти `ValueHunt.exe`
4. Открой [http://127.0.0.1:8100](http://127.0.0.1:8100)

> Если предпочитаешь запуск из исходников: `git clone`, `pip install -r requirements.txt`, `start_webui.py`

## Возможности

| Возможность | Статус |
|-------------|--------|
| Парсинг HH.ru (BeautifulSoup) | ✅ Работает |
| UES Calculator (7 групп критериев, Gates A/B) | ✅ Работает |
| Web UI (дашборд, профили, матрица, вакансии, контакты) | ✅ Работает |
| Мульти-профили (несколько конфигураций поиска) | ✅ Работает |
| CRM контактов + календарь взаимодействий | ✅ Работает |
| Корзина с soft-delete | ✅ Работает |
| Сбор данных о компаниях | ✅ Работает |
| Drop-out scoring | ✅ Работает |
| LLM-анализ вакансий (DeepSeek) | 🚧 Заглушка |
| Telegram-бот | 🚧 Заглушка |
| Авто-отклик через Playwright | 🚧 Заглушка |
| Адаптивный скоринг (LLM-агент) | 🚧 В разработке |

## Скриншоты

**Дашборд** — воронка отклика, категории скоринга, динамика публикаций:

![Дашборд](docs/screenshots/dashboard.png)

**Вакансии** — таблица с баллами и категориями UES:

![Вакансии](docs/screenshots/vacancies.png)

**Контакты** — CRM HR-контактов и календарь взаимодействий:

![Контакты](docs/screenshots/contacts.png)

**Матрица оценки** — критерии, веса и группы скоринга:

![Матрица оценки](docs/screenshots/matrix.png)

**Отчёт** — статистика по периодам:

![Отчёт](docs/screenshots/report.png)

## Технологии

- **Python 3.12+** + **FastAPI** — бэкенд
- **SQLite** — всё локально, без сервера
- **BeautifulSoup + lxml** — парсинг HH.ru
- **Jinja2** — шаблоны HTML
- **PyInstaller** — готовая сборка `.exe`
- **DeepSeek API** — LLM-анализ (планируется)

## Структура

```
ValueHunt/
├── src/
│   ├── scanner.py          # Парсинг HH.ru
│   ├── ues.py              # UES Calculator
│   ├── collector.py        # Данные о компаниях
│   ├── db.py               # SQLite — схемы, CRUD
│   └── web/                # FastAPI + HTML/JS
├── config/                 # profile.json, matrix.yaml
├── data/                   # valuehunt.db (создаётся)
├── docs/                   # USER.md, ADMIN.md, DEV.md
└── dist/ValueHunt/         # Готовая сборка
```

## Почему ValueHunt?

- **Для себя**: поиск работы — это процесс, который можно и нужно автоматизировать
- **Open-source**: никаких подписок, всё локально, твои данные — твоё дело
- **РФ-доступно**: DeepSeek API, HH.ru, всё работает без VPN и зарубежных карт
- **Гибкость**: матрица ценностей настраивается под любого кандидата, не только IT PM

## Лицензия

MIT

---

> Полное ТЗ, архитектура и API — [docs/SPEC.md](docs/SPEC.md). Разработчику — [docs/DEV.md](docs/DEV.md). Пользователю — [docs/USER.md](docs/USER.md). Администратору — [docs/ADMIN.md](docs/ADMIN.md). (оценка 1-10 + вес внутри группы). Финальный Score = средневзвешенное по группам.

### 3.2. Группы критериев

#### Группа 1. Роль и содержание работы (вес группы: 10)

| Подкритерий | Вес | Описание |
|------------|:---:|----------|
| Роль PM/PO (гибрид) | 10 | Совмещение Project и Product Owner |
| Enterprise-масштаб | 9 | Крупные заказчики, сложные интеграции |
| Продуктовое влияние | 9 | Бэклог, roadmapping, дашборды метрик |
| Полный цикл | 8 | От discovery до пост-поддержки |

#### Группа 2. Условия работы (вес группы: 10)

| Подкритерий | Вес | Описание |
|------------|:---:|----------|
| Удалёнка 100% | 10 | Работа из любой точки |
| Гибрид (офис по необходимости) | 8 | Разовые встречи/командировки |
| Офис Москва | 4 | Только если поездки раз в 2-4 недели |
| Командировки | 6 | Готовность к поездкам |

#### Группа 3. Финансы (вес группы: 9)

| Подкритерий | Вес | Описание |
|------------|:---:|----------|
| Фикс от 250k net | 10 | Минимальный порог |
| Вилка указана | 7 | Прозрачность компании |
| IT-аккредитация | 6 | Налоговые льготы |
| ДМС (стоматология) | 5 | Полис с первого дня |

#### Группа 4. Компания и репутация (вес группы: 8)

| Подкритерий | Вес | Описание |
|------------|:---:|----------|
| Рейтинг HH (Top-50/100) | 8 | Зрелость HR-функции |
| Dream Job / отзывы | 7 | Реальная культура |
| Возраст компании (3+ лет) | 6 | Стабильность |
| Медийность CEO/команды | 7 | Прозрачность |

#### Группа 5. Карьера и развитие (вес группы: 8)

| Подкритерий | Вес | Описание |
|------------|:---:|----------|
| Карьерная лестница | 9 | Грейды, понятный рост |
| Обучение за счёт | 8 | Конференции, курсы, сертификации |
| Менторство/наставничество | 7 | Культура развития |
| Трек до PPM/CPO | 9 | Долгосрочный путь |

#### Группа 6. Технологический стек (вес группы: 7)

| Подкритерий | Вес | Описание |
|------------|:---:|----------|
| AI/ML-инструменты | 8 | Актуальность |
| Data / BI / DWH | 8 | Основная экспертиза |
| Современный стек | 7 | CI/CD, Docker, K8s, не legacy |
| Продуктовые метрики | 8 | Data-driven культура |

#### Группа 7. Культура и процессы (вес группы: 7)

| Подкритерий | Вес | Описание |
|------------|:---:|----------|
| Зрелые Agile-процессы | 9 | Не хаос, не бюрократия |
| Прозрачность в найме | 8 | Быстрая обратная связь |
| Отсутствие аутстаффа | 7 | Прямой найм |
| Соцпакет | 6 | ДМС, спорт, питание |

### 3.3. Формула расчёта

```
Score_группы = Σ(оценка_подкритерия × вес_подкритерия) / Σ(весов_подкритериев)
Финальный_Score = Σ(Score_группы × вес_группы) / Σ(весов_групп)
```

### 3.4. Шкала решений

| Score | Категория | Действие |
|:-----:|:---------:|----------|
| 8.5-10 | A | Идеальное попадание → уведомление пользователю, подготовка кастомного письма |
| 7.0-8.4 | Б | Сильное совпадение → уведомление + автошаблон с подтверждением |
| 6.0-6.9 | В | Компромисс → автоотклик по шаблону, уведомление в ленту |
| < 6.0 | — | Пропустить |

---

## 4. Модули системы

### 4.1. Модуль анкетирования

- Интерфейс заполнения матрицы (группы + подкритерии)
- Привязка резюме HH (по ID)
- Настройка степени автоматизации (авто/вручную по категориям)
- Сохранение профиля в JSON/YAML

### 4.2. Модуль сбора вакансий (кронер)

**Периодичность**: каждые 4 часа (настраивается)

**Источники**:
- HH API (основной): поиск по фильтрам, полное описание вакансии
- Dream Job: отзывы о компании (если доступен парсинг)
- Сайт компании: описание, ценности (LLM-анализ)

**Фильтры поиска** (из профиля пользователя):
- Регионы: СПб + Москва
- Должности: Руководитель проектов, PM, Product Manager, PO, Delivery Manager, Enterprise PM
- Проф. роли: 107, 73
- Отрасли: 7.540, 7.539
- Ключевые слова из набора (Enterprise, Senior, Head of Delivery и т.д.)

### 4.3. Модуль оценки (LLM-агент)

**Задачи LLM**:
1. Семантический анализ вакансии — о чём реально эта роль
2. Извлечение: формат работы (реальная удалёнка?), стек, культура
3. Проверка компании: рейтинг, отзывы, новости
4. Расчёт Score по матрице
5. Присвоение категории (A/Б/В/мимо)

**Модель**: DeepSeek через OpenRouter (бесплатная квота)

### 4.4. Модуль отклика (автовеб)

**Технология**: Playwright (headless-браузер)

**Действия**:
1. Авторизация на HH (сессия, cookie)
2. Выбор резюме (01 или 03) по типу вакансии
3. Подстановка сопроводительного письма (шаблон 01 или 03 + адаптация)
4. Отклик
5. Запись результата в БД

**Триггеры**:
- Score В (6.0-6.9) → автоотклик без подтверждения
- Score Б (7.0-8.4) → подготовить, ждать одобрения
- Score A (8.5-10) → уведомление + ручной отклик

### 4.5. Модуль CRM / Трекер

**Сущности**:
- Вакансия: ссылка, компания, Score, статус, даты
- Контакт: HR, имя, телефон, Telegram, LinkedIn
- Коммуникация: дата, тип (отклик/письмо/звонок), заметки

**Статусы**:
- Новое → Откликнуто → Приглашение → Собеседование (1/2/N) → Оффер / Отказ / Архив
- Напоминание (через N дней написать HR)

**Источник контактов**:
- Парсинг из текста вакансии (если указан HR)
- Ручное добавление

### 4.6. Модуль рефлексии

**Еженедельный автоотчёт**:
- Сколько новых вакансий найдено
- Сколько откликов (авто/ручные)
- Конверсия: отклик → приглашение → собеседование → оффер
- Архив: какие вакансии закрыты, какие висят без ответа
- Статистика отказов (формулировки, причины)
- Графики конверсии (Chart.js на GitHub Pages)
- Рекомендации: сменить фильтр, обновить резюме

---

## 5. Пользовательские сценарии

### Сценарий 1. Первичная настройка (единожды)
1. Открыть десктоп-приложение
2. Заполнить матрицу ценностей (веса групп, оценки подкритериев)
3. Указать ID резюме HH
4. Выбрать степень автоматизации
5. Сохранить профиль

### Сценарий 2. Ежедневная работа
1. Получить уведомление в Telegram: «Найдено 3 новых вакансии»
2. Открыть дашборд
3. Просмотреть топ по Score: A-1, Б-2, автоотклик-0
4. Для A/Б: прочитать LLM-анализ, отредактировать письмо, нажать «Откликнуться»
5. Playwright делает отклик, статус → «Откликнуто»

### Сценарий 3. Еженедельная рефлексия
1. Получить автоотчёт в Telegram / на почту
2. Просмотреть графики: неделя к неделе
3. Прочитать рекомендации: «Фильтр по ЗП даёт 0 результатов — пора расширить»
4. Применить изменения в фильтре

### Сценарий 4. CRM (работа с контактами)
1. Открыть карточку вакансии
2. Увидеть контакт HR (если найден)
3. Нажать «Напомнить через 3 дня»
4. Получить напоминание: «Напомнить HR по вакансии YADRO»
5. Написать HR в Telegram по шаблону

---

## 6. Прототипирование (GitHub Pages / Render / Telegram)

### 6.1. Telegram Bot (первичный интерфейс)

**Команды**:
- `/start` — приветствие, краткая инструкция
- `/today` — новые вакансии за сегодня с Score
- `/stats` — статистика за неделю
- `/top` — top-5 вакансий по Score
- `/companies` — статусы по целевым компаниям
- `/remind` — напоминания (просроченные)

### 6.2. GitHub Pages (дашборд статистики)

- Статический сайт (HTML + Chart.js)
- Автообновление через GitHub Actions
- Графики: конверсия, новые вакансии по дням, статусы откликов

### 6.3. Desktop App (Tauri)

Поверх GitHub Pages / локальной БД:
- Полноценная анкета
- Редактирование профиля
- CRM с таблицами
- Календарь напоминаний
- Локальный запуск Python-скриптов (через subprocess или встроенный API)

---

## 7. Технический стек

| Компонент | Решение | Обоснование |
|-----------|---------|-------------|
| Язык бэкенда | Python | HH API, LLM, Playwright — всё на Python |
| Фреймворк | FastAPI | Легковесный REST |
| База данных | SQLite | Один пользователь, не надо сервера |
| Очередь | GitHub Actions (cron) | Бесплатно, без Redis |
| Автоматизация | Playwright | Современнее Selenium |
| LLM | DeepSeek (прямой API) | Доступен в РФ, есть бесплатный tiers |
| Десктоп | Tauri (Rust + HTML/JS) | Лёгкий, как TV Hamsters |
| Кроны | GitHub Actions | Бесплатно, 2000 мин/мес |
| Бот | python-telegram-bot | Бесплатно |
| Хостинг | GitHub Pages | Статика бесплатно |
| Раннер | GitHub Actions / self-hosted | Если нужно больше ресурсов |

---

## 8. MVP (2-3 недели)

### Этап 1. Базовый парсер + оценка
- Скрипт на Python: HH API → парсинг вакансий по фильтрам
- Матрица оценки (YAML/JSON): 7 групп, подкритерии, веса
- Вывод Score для каждой вакансии
- **Результат**: консольный скрипт, выдающий JSON с оценками

### Этап 2. Telegram-бот + GitHub Actions cron
- Бот: `/today` — новые вакансии с Score
- GitHub Actions: каждые 4 часа → парсинг → оценка → уведомление в Telegram
- **Результат**: ежедневные уведомления в телефоне

### Этап 3. База данных (SQLite)
- Схема: таблицы vacancies, contacts, communications, analytics
- Сохранение истории всех вакансий
- **Результат**: данные не теряются

### Этап 4. Playwright-отклик
- Скрипт: авторизация HH → отклик по шаблону
- Привязка к Score В (авто)
- **Результат**: автоотклики на нейтральные вакансии

### Этап 5. Дашборд на GitHub Pages
- Статистика, графики
- Обновление через GitHub Actions
- **Результат**: визуальная аналитика

### Этап 6. Desktop App (Tauri)
- Анкета + CRM
- Локальное управление профилем
- **Результат**: полноценное приложение

---

## 9. Структура БД (SQLite) — MVP

```sql
-- Профиль пользователя
CREATE TABLE profile (
    id INTEGER PRIMARY KEY,
    hh_resume_id TEXT,
    telegram_chat_id TEXT,
    auto_respond_below_score REAL DEFAULT 6.9,
    created_at TEXT
);

-- Матрица критериев (JSON — гибкая схема)
CREATE TABLE matrix (
    id INTEGER PRIMARY KEY,
    groups_json TEXT,  -- весь набор групп и весов
    version INTEGER,
    updated_at TEXT
);

-- Вакансии
CREATE TABLE vacancies (
    id INTEGER PRIMARY KEY,
    hh_id TEXT UNIQUE,
    title TEXT,
    company TEXT,
    url TEXT,
    salary_from INTEGER,
    salary_to INTEGER,
    description TEXT,        -- полный текст
    skills_json TEXT,        -- ключевые навыки
    score REAL,              -- итоговый балл
    category TEXT,           -- A/Б/В/мимо
    llm_analysis TEXT,       -- анализ от LLM
    status TEXT DEFAULT 'new',  -- new/responded/invited/interview/offer/rejected/archived
    created_at TEXT,
    responded_at TEXT,
    archived_at TEXT
);

-- Контакты HR
CREATE TABLE contacts (
    id INTEGER PRIMARY KEY,
    vacancy_id INTEGER,
    name TEXT,
    role TEXT,
    phone TEXT,
    telegram TEXT,
    linkedin TEXT,
    notes TEXT,
    FOREIGN KEY (vacancy_id) REFERENCES vacancies(id)
);

-- Коммуникации
CREATE TABLE communications (
    id INTEGER PRIMARY KEY,
    vacancy_id INTEGER,
    contact_id INTEGER,
    type TEXT,      -- response/message/call/interview
    direction TEXT, -- outbound/inbound
    content TEXT,   -- текст письма/сообщения
    reminder_date TEXT,  -- когда напомнить
    created_at TEXT,
    FOREIGN KEY (vacancy_id) REFERENCES vacancies(id),
    FOREIGN KEY (contact_id) REFERENCES contacts(id)
);

-- Аналитика
CREATE TABLE analytics (
    id INTEGER PRIMARY KEY,
    date TEXT,
    new_vacancies INTEGER,
    auto_responses INTEGER,
    manual_responses INTEGER,
    invitations INTEGER,
    interviews INTEGER,
    offers INTEGER,
    rejections INTEGER,
    recommendations TEXT  -- текст рекомендаций от LLM
);
```

---

## 10. Prompts для LLM-агента (MVP)

### Prompt анализа вакансии

```
Ты — HR-аналитик. Проанализируй вакансию и компанию для кандидата IT PM/PO.

Данные:
- Название вакансии: {title}
- Компания: {company}
- Описание: {description}
- Требования: {requirements}
- Ключевые навыки: {skills}

Оцени по шкале 1-10:
1. Роль PM/PO (гибрид)?
2. Enterprise-масштаб?
3. Реальная удалёнка или декларативная?
4. Зрелость процессов (Agile, метрики)?
5. Интересный стек (AI/ML, BI)?
6. Прозрачность (указана ЗП? описана культура?)
7. Риски (аутстафф? бюрократия? токсичность?)

Верни JSON:
{
  "role_type": "PM/PO/hybrid/other",
  "remote_real": true/false,
  "enterprise_scale": 1-10,
  "culture_notes": "...",
  "risks": ["..."],
  "score_estimate": 1-10,
  "summary": "2-3 предложения"
}
```

### Prompt еженедельной рефлексии

```
Ты — HRBP. Проанализируй статистику поиска работы за неделю.

Данные:
{analytics_data}

Оцени:
1. Какие тренды?
2. Что изменилось по сравнению с прошлой неделей?
3. Что работает, что нет?
4. Рекомендации:
   - Менять фильтры?
   - Обновлять резюме?
   - Менять ЗП-ожидания?
   - Другие каналы поиска?

Верни {recommendations_text}
```

---

## 11. Файловая структура репозитория (MVP)

```
jobmatch/
├── .github/
│   └── workflows/
│       └── daily_scanner.yml     # GitHub Actions: крон каждые 4 часа
├── src/
│   ├── scanner.py                # HH API парсер
│   ├── scorer.py                 # Матрица оценки
│   ├── llm_agent.py              # LLM-анализ
│   ├── bot.py                    # Telegram-бот
│   ├── browser_agent.py          # Playwright-отклик
│   ├── db.py                     # SQLite модели
│   └── reporter.py               # Еженедельный отчёт
├── config/
│   ├── profile.json              # Профиль пользователя
│   └── matrix.yaml               # Матрица критериев
├── data/
│   └── jobmatch.db               # SQLite БД
├── docs/                         # GitHub Pages дашборд
│   ├── index.html
│   └── stats.js
├── requirements.txt
└── README.md
```

---

## 12. План на первый релиз (v0.1)

1. **Сканер** — парсинг HH API, сохранение в SQLite
2. **Scorer** — расчёт Score по матрице (первая версия без LLM, на keywords)
3. **Telegram-бот** — уведомление о новых вакансиях с Score
4. **GitHub Actions** — автоматический запуск раз в 4 часа
5. **Дашборд** — статическая страница с базовой статистикой

---

## 13. Ограничения (open-source / РФ)

### По стеку
использовать можем только доступные опенсорс решения и бесплатные сервисы, доступные в РФ. Ниже примеры но не жесткие требования:
- **Только open-source**: Python, FastAPI, Playwright, Tauri, SQLite — все MIT/BSD лицензии
- **Бесплатные сервисы**: GitHub Actions (2000 мин/мес), GitHub Pages, Telegram Bot API
- **Render исключён**: может блокировать РФ, требует иностранную карту. Замена — self-hosted раннер GitHub Actions или запуск на своей машине через cron (Task Scheduler Windows)

### По AI
- **DeepSeek API** — доступен напрямую из РФ, регистрация по российскому номеру, есть бесплатный tiers (достаточно для анализа ~50-100 вакансий/день)
- **Альтернативы для РФ**: YandexGPT (бесплатный tiers через Yandex Cloud), GigaChat от Сбера
- **OpenRouter исключён** — может блокировать РФ

### По работе HH
- **HH API** — открытый, доступен из РФ, 2000 запросов/сутки бесплатно
- **Отклики** — только через Playwright (браузер). HH API не позволяет откликаться
- **Данные**: все персональные данные (сессии, куки HH) хранятся локально, не передаются
